...
class Tug(object):
    ...
    def move(self, dt, t):
        ...
        remaining_distance = round(math.sqrt((xy_to[0] - self.position[0]) ** 2 + (xy_to[1] - self.position[1]) ** 2),
                                   2)
        ...
        if remaining_distance != 0:
            ...
            remaining_distance = round(
                math.sqrt((xy_to[0] - self.position[0]) ** 2 + (xy_to[1] - self.position[1]) ** 2), 2)
    ...
    def assign_ac(self, ac):
        ...
        if self.status == "charging" or self.status == "moving_charging":
            ...
            # can't be assigned
            raise Exception(
                f"Tug {self.id} cannot be assigned to aircraft {ac.id} as it is already assigned to another aircraft.")
    ...
    def attach_to_ac(self, t):
        ...
        if self.assigned_ac != None and self.attached_ac == None:
            ...
            raise Exception(
                f"Tug {self.id} cannot attach to aircraft {self.assigned_ac.id} as it is already attached to another aircraft. Current situation: assigned to {self.assigned_ac} and attached to {self.attached_ac}")
    ...
    def plan_free_path(self, heuristics, time_start):
        ...
        if self.assigned_ac != None:
            if self.position == self.assigned_ac.position:
                ...
                self.assigned_ac.acknowledge_attach(self.id)
                return
        success, path = simple_single_agent_astar(self.nodes_dict, self.from_to[0], self.goal, heuristics, time_start,
                                                  [], f"tug{self.id}")
        if success:
    ...
    def calculate_free_path(self, from_node, to_node, heuristics, time_start):
        ...
        """
        success, path = simple_single_agent_astar(self.nodes_dict, from_node, to_node, heuristics, time_start, [],
                                                  f"tug{self.id}")
        if success:
            return path
        else:
    ...
    def make_bid(self, ac, heuristics, bid_time):
        ...
        if self.status == "ready":
            ...
            self.calculate_free_path(self.assigned_ac.start, self.assigned_ac.goal, heuristics, time_to_arrive_current)[
                -1][1]
            time_to_reach_ac = \
            self.calculate_free_path(self.assigned_ac.goal, ac.start, heuristics, time_to_deliver_current)[-1][1]
            ...
            time_to_reach_ac = \
            self.calculate_free_path(self.from_to[0], ac.start, heuristics, bid_time + time_to_charge)[-1][1]
            return time_to_reach_ac
        elif self.status == "moving_charging" and self.assigned_ac == None:
            # calculate time left to charge
            time_to_arrive_current = self.path_to_goal[-1][1]
            time_to_charge = ((100 - self.energy) / self.charge_speed)
            time_to_reach_ac = self.calculate_free_path(self.path_to_goal[-1][0], ac.start, heuristics,
                                                        time_to_charge + time_to_arrive_current)[-1][1]
            return time_to_reach_ac
